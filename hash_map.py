...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        node = self.buckets.get_at_index(my_index).contains(key)
        if node is None:
            self.buckets.get_at_index(my_index).insert(key, value)
            self.size +=1
        else:
            node.value = value
    ...
    def table_load(self) -> float:
        """
        This method returns the load factor for this hash table
        """
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        """
        if new_capacity < 1:  # Do nothing if new capacity is less than 1
            return

        temp_table = HashMap(new_capacity, self.hash_function)
        for i in range(self.capacity):
